...
OUTPUT_DIR = "benchmark_output"
COMPILER = "g++"
SOURCE_FILE = "AdvCG_Final_MIS.cc" # 原始主程式
SOURCE_INPUT = "input.txt" 
TARGET_INPUT = os.path.join(OUTPUT_DIR, "input_benchmark.txt")

# [設定] 解析度 2048x2048 (確保 BVH 優勢明顯)
TARGET_WIDTH = 2048         
TARGET_HEIGHT = 2048        

print(f"🔥 [Benchmark V24 Final] Layout Fix & Gauge Delta")
print(f"   Mode: Linear Scan (No BVH) vs. BVH Enabled")
print(f"   Config: Resolution={TARGET_WIDTH}x{TARGET_HEIGHT}, Only MIS Image")
print(f"   Fix: Heatmap 1:1 Aspect Ratio, Gauge shows 'Speedup' AND 'Delta'")
...
def prepare_scene():
    if not find_input_file(): return False
    print(f"⚙️ [1/5] Preparing Scene (Resolution {TARGET_WIDTH}x{TARGET_HEIGHT})...")
...
# ================= 2. 生成原始碼 (Linear/BVH, Only MIS) =================
def generate_source_files():
    print("🛠 [2/5] Generating Variants (Only MIS)...")

    headers = ["Speed_up.h", "tracer_camera.h", "tracer_light_source.h", 
    ...
    # --- 版本 A: Linear Scan (關閉 BVH) ---
    src_linear = re.sub(r'(build_scene_bvh\s*\(\s*&?scene\s*\)\s*;)', r'// \1', src_content)
    
    with open(os.path.join(OUTPUT_DIR, "Linear.cc"), "w") as f:
        f.write(src_linear)

    # --- 版本 B: BVH Enabled ---
    with open(os.path.join(OUTPUT_DIR, "BVH.cc"), "w") as f:
        f.write(src_content)
...
def analyze_results(t_lin, t_bvh):
    ...
    try:
        ...
        if arr_lin.shape == arr_bvh.shape:
            ...
            diff = np.round(diff).astype(np.uint8) # 0-255 Integer
    ...
    # 2. Gauge (With Delta)
    # mode="gauge+number+delta" 讓它同時顯示數值與變化量
    fig.add_trace(go.Indicator(
        mode="gauge+number+delta",
        value=speedup,
        delta={
            'reference': 1.0,      # 基準值 1.0
            'relative': False,     # 顯示絕對差值 (Value - 1.0)
            'valueformat': '.2f'   # 小數點兩位
        }, 
    ...
    # 3. Heatmap (1:1 Aspect Ratio Fix)
    if diff_valid:
        ...
        fig.add_trace(go.Heatmap(
            ...
            colorbar=dict(
                title=dict(text="Diff Value (0-10)", font=dict(size=18)), 
                tickfont=dict(size=14), 
                len=0.75, y=0.38, yanchor="middle"
            )
        ), row=2, col=1)
        
        # [關鍵修正] 強制 1:1
        # scaleanchor="x" 綁定 Y 軸比例等於 X 軸
        # scaleratio=1 確保像素是正方形
        fig.update_yaxes(
            autorange="reversed", 
            scaleanchor="x", 
            scaleratio=1, 
        ...
        fig.update_xaxes(row=2, col=1, tickfont=dict(size=14))
    else:
        fig.add_annotation(text="Image diff not available", xref="paper", yref="paper", showarrow=False, row=2, col=1)

    # Layout Adjustment
    # 設定固定寬高 width=1600, height=1600 以確保正方形畫布
    fig.update_layout(
        width=1600,
        height=1600,
        title_text="Benchmark V24: Linear Scan vs BVH", 
        title_font=dict(size=24), 
        font=dict(size=16),
        template="plotly_dark",
        autosize=False # 禁止自動縮放，確保 1:1
    )
...
def compile_and_run():
    ...
    # 1. Compile Linear
    cmd_lin = [COMPILER] + flags + [os.path.join(OUTPUT_DIR, "Linear.cc"), "-o", EXE_LINEAR]
    subprocess.run(cmd_lin, check=True)

    # 2. Compile BVH
    cmd_bvh = [COMPILER] + flags + [os.path.join(OUTPUT_DIR, "BVH.cc"), "-o", EXE_BVH]
    subprocess.run(cmd_bvh, check=True)

    print("\n🏁 [4/5] Running Benchmark...")

    # Run Linear
    print(f"\n>>> Running Linear Scan (Expect Slow)...")
    t0 = time.time()
    subprocess.run([os.path.abspath(EXE_LINEAR), "input_benchmark.txt"], cwd=OUTPUT_DIR)
    t_lin = time.time() - t0
    ...
    t0 = time.time()
    subprocess.run([os.path.abspath(EXE_BVH), "input_benchmark.txt"], cwd=OUTPUT_DIR)
    t_bvh = time.time() - t0
