...
inline int scene_occluded_old(const Scene* scene,
                          const point3& p,
                          const vec3& wi,
                          double max_dist) {
    ...
    shadow_ray.dir  = wi;
    HitRecord rec;
    if (hit_scene_optimized(scene, &shadow_ray, EPS, max_dist - EPS, &rec)) {
        return 1;
    }
    return 0;
}
...
inline int trace_to_light(const Scene* scene,
    ...
    r.dir  = wi;
    HitRecord rec;
    if (!hit_scene_optimized(scene, &r, EPS, DBL_MAX, &rec)) return 0;
